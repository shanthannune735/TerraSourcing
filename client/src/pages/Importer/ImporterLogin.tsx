...
import { importerLogin } from "../../services/importerServices";

export default function ImporterLogin(){
  ...
  async function handleLogin(e: React.MouseEvent<HTMLButtonElement, MouseEvent>){
    e.preventDefault()
    try {
      const { token } = await importerLogin({email: inputData.email, password: inputData.password})
      if(token){
        navigate("/importer/login-successful")
      } else {
  ...
  return(
    ...
    <h1 className="text-center text-4xl font-semibold pb-8">Importer Login</h1>
    <div className="max-w-[560px] border p-4 md:p-8 rounded-2xl mx-auto  flex flex-col gap-8 bg-white">
      ...
      <div>
        <button onClick={handleLogin} className="bg-primary py-2 text-white rounded-md w-full">Login</button>
      </div>
